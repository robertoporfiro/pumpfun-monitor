...
def _find_target_pair(pairs: list, mint_address: str) -> Optional[Dict[str, Any]]:
    ...
    for i, pair in enumerate(pairs):
        ...
        logger.debug(f"[{mint_address}] Avaliando par {i}: {pair.get('pairAddress','?')} ({ (pair.get('baseToken', {}) or {}).get('symbol', '?')}/{quote_token.get('symbol', '?')}), Liq: ${liquidity_usd:.2f}")
        if is_sol_pair: sol_pair = pair; logger.debug(f"[{mint_address}] Encontrado par SOL: {pair.get('pairAddress','?')}"); break
    ...
    if not target_pair: logger.warning(f"[{mint_address}] Não encontrou par alvo DexScreener."); return None
...
def _parse_market_data_from_pair(pair_data: Dict[str, Any], mint_address: str) -> Optional[Dict[str, Any]]:
    ...
    # --- CORPO DA FUNÇÃO INDENTADO CORRETAMENTE ---
    ...
    try:
        ...
        extracted["buys_m5"] = int(txns_m5.get("buys")) if txns_m5.get("buys") is not None else None
        ...
        if missing: logger.warning(f"[{mint_address}] Dados essenciais ausentes DexScreener: {missing}. Par: {pair_data.get('pairAddress')}"); return None
        logger.debug(f"[{mint_address}] Dados DexScreener OK: P={extracted['price_usd']:.8f}, V5m={extracted['volume_m5']:.2f}, B5m={extracted['buys_m5']}, S5m={extracted['sells_m5']}, H1%={extracted['price_change_h1']:.2f}, FDV={extracted['fdv']:,.0f}"); return extracted
    except (ValueError, TypeError, KeyError) as e: logger.error(f"[{mint_address}] Erro conversão/extração dados par DexScreener: {e}. Par: {pair_data.get('pairAddress')}", exc_info=False); return None
    except Exception as e: logger.error(f"[{mint_address}] Erro inesperado _parse_market_data: {e}. Par: {pair_data.get('pairAddress')}", exc_info=True); return None
...
async def fetch_market_data(mint_address: str, session: aiohttp.ClientSession) -> Optional[Dict[str, Any]]:
    ...
    # --- CORPO DA FUNÇÃO INDENTADO CORRETAMENTE ---
    url = f"https://api.dexscreener.com/latest/dex/tokens/{mint_address}"; logger.debug(f"[{mint_address}] Fetching DexScreener: {url}")
    try:
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=config.API_TIMEOUT)) as response:
            status = response.status; text = await response.text(); logger.debug(f"[{mint_address}] DexScreener status: {status}")
            if status == 200:
                try: data = json.loads(text)
                except json.JSONDecodeError as e: logger.error(f"[{mint_address}] Erro JSON DexScreener: {e}. Resp:{text[:500]}"); return None
                pairs = data.get("pairs");
                if not pairs or not isinstance(pairs, list): logger.warning(f"[{mint_address}] Campo 'pairs' inválido DexScreener: {data}"); return None
                target = _find_target_pair(pairs, mint_address); return _parse_market_data_from_pair(target, mint_address) if target else None
            elif status == 404: logger.warning(f"[{mint_address}] Token não encontrado DexScreener (404)."); return None
            elif status == 429: logger.warning(f"[{mint_address}] Rate limit DexScreener (429)."); return None
            else: logger.error(f"[{mint_address}] Erro DexScreener: Status {status}. Resp: {text[:500]}"); return None
    except Exception as e: logger.error(f"[{mint_address}] Exceção fetch_market_data: {type(e).__name__} - {e}", exc_info=(config.LOG_LEVEL_NAME=="DEBUG")); return None
    # --- FIM DA INDENTAÇÃO CORRETA ---
...
def _check_market_criteria(m: Dict[str, Any], init_p: float, mint: str, i: int) -> Tuple[bool, str]:
    ...
    log=(f"  P:{m['price_usd']:.6f}(>{init_p*(1.0-config.MARKET_PRICE_DROP_TOLERANCE):.6f})->{'OK' if p_ok else 'F'}|"
         f"V5m:{m['volume_m5']:.0f}(>{config.MARKET_MIN_VOLUME_M5:.0f})->{'OK' if v_ok else 'F'}|"
...
# --- monitor_market_activity (Revertido para usar set global) ---
async def monitor_market_activity(mint_address: str, initial_price: float, session: aiohttp.ClientSession, state: StateManager):
    ...
    try:
        errors = 0; max_errors = 5; i = 0; buy_attempted = False; last_log = ""; market_criteria_met_on_exit = False
        while time.monotonic() < deadline and not buy_attempted:
            ...
            if mint_address not in tokens_being_monitored or getattr(current_task, '_must_cancel', False):
                 logger.info(f"[{mint_address}] Monitoramento interrompido (não no set ou cancelado)."); break
            if i > 1:
            ...
            logger.debug(f"[{mint_address}] Iteração {i} monitoramento...")
            mkt_data = await fetch_market_data(mint_address, session)
            if mkt_data:
                errors = 0
                try:
                    criteria_met, last_log = _check_market_criteria(mkt_data, initial_price, mint_address, i)
                    market_criteria_met_on_exit = criteria_met
                except Exception as e: logger.error(f"[{mint_address}] Erro check critérios (Iter {i}): {e}", exc_info=True); criteria_met = False; market_criteria_met_on_exit = False
                if criteria_met:
                    logger.info(f"[{mint_address}] CRITÉRIOS ATINGIDOS (Iter {i})! Enviando ordem Sniperoo...")
                    buy_attempted = True
                    buy_ok = await place_sniperoo_buy_order(session=session, mint_address=mint_address) # Chama função neste módulo
                    if buy_ok: logger.info(f"[{mint_address}] Ordem IMEDIATA enviada. Encerrando monitor.")
                        ...
                        except Exception as state_err: logger.error(f"[{mint_address}] Erro add pending pós falha envio: {state_err}")
                    break # Sai do loop
                else: logger.debug(f"[{mint_address}] Critérios não atingidos (Iter {i}).")
            else: # Falha fetch
                errors += 1; logger.warning(f"[{mint_address}] Falha fetch DexScreener (Erro {errors}/{max_errors}).")
                market_criteria_met_on_exit = False
                if errors >= max_errors:
                    logger.error(f"[{mint_address}] Desistindo monitor - {max_errors} erros DexScreener.")
                    try: state.add_pending_token({"mint": mint_address, "ts": datetime.now(timezone.utc).isoformat(),"reason": f"{max_errors} erros DexScreener"})
                    except Exception as state_err: logger.error(f"[{mint_address}] Erro add pending pós erros Dex: {state_err}")
                    break
        if time.monotonic() >= deadline and not buy_attempted: logger.info(f"[{mint_address}] Timeout monitor ({config.MARKET_MONITOR_DURATION}s) sem atingir critérios.")
        ...
        except Exception as se: logger.error(f"[{mint_address}] Erro add pending pós erro: {se}")
    finally:
        tokens_being_monitored.discard(mint_address)
        logger.debug(f"[{mint_address}] Removido do monitoramento ativo (finally).")
    # --- FIM DA INDENTAÇÃO CORRETA ---

# --- _build_sniperoo_payload e place_sniperoo_buy_order (Definições corretas) ---
...
async def place_sniperoo_buy_order(session: aiohttp.ClientSession, mint_address: str) -> bool:
    ...
    try:
        ...
        async with session.post(end, json=payload, headers=h, timeout=tout) as resp:
            ...
            if 200 <= stat < 300:
                ...
                err_msg="Erro desconhecido"; err_det=data.get("error"); msg=data.get("message")
    ...
    except Exception as e: logger.error(f"[{mint_address}] Exceção place_sniperoo_buy_order: {type(e).__name__} - {e}. URL:{end}", exc_info=(config.LOG_LEVEL_NAME=="DEBUG")); return False
    # --- FIM DA INDENTAÇÃO CORRETA ---