...
import os
import json
import atexit # Import atexit para remove_pid_file
...
logging.basicConfig(
    ...
    # Sem logging.Formatter.converter = time.gmtime
)
...
async def main():
    ...
    try:
        ...
        atexit.register(remove_pid_file) # Tenta remover na saída normal/atexit
    except Exception as e: logger.error(f"Não criou PID file {_pid_file_path}: {e}")

    try: config.log_initial_config() # Loga configs
    ...
    # --- Instancia StateManager com 3 argumentos ---
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            for task in done: # Logar exceções
    ...
    except aiohttp.ClientError as e: logger.critical(f"Erro criar ClientSession: {e}")
    except Exception as e: logger.critical(f"Erro não tratado main: {e}", exc_info=True)

# --- shutdown_handler (Atualizado para chamar remove_pid_file) ---
def shutdown_handler(signum, frame):
    ...
    if _state_manager_instance and not _state_manager_instance.is_final_save_done():
        ...
        try:
            processed=list(_state_manager_instance.get_processed_tokens_copy()); pending=list(_state_manager_instance.get_pending_tokens_copy)
    ...
    else: logger.debug("Salvamento final já feito ou StateManager indisponível.")

    remove_pid_file() # Chama a função de remoção do PID
    ...
    try: # Tenta parar o loop
        ...
        # Usar call_soon_threadsafe é mais seguro de signal handlers
        loop.call_soon_threadsafe(asyncio.ensure_future, stop_tasks())
...
if __name__ == "__main__":
    ...
    try: asyncio.run(main())
        ...
        remove_pid_file() # Garante tentativa de remoção no final
        logger.info("Processo monitoramento formalmente encerrado.")